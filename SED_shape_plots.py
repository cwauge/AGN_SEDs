...
class SED_shape_Plotter(Plotter):
    ...
    def shape_1bin_h(self, savestring, median_x=[np.nan], median_y=[np.nan], wfir=[[np.nan]], ffir=[[np.nan]], uvslope=None, mirslope1=None, mirslope2=None, Median_line=True, FIR_med=True, FIR_upper='upper lims'):
        ...
        if Median_line:
            ...
            if FIR_med:
                x_connect, y_connect = self.median_sed(median_x4, median_y4, connect_point=True, Bin=True, bin_in=b4, lw=3)
        ...
        lc5 = self.multilines(x4,y4,L4,cmap=cmap,lw=1.5,alpha=0.7,rasterized=True)
        ...
        if Median_line:
            ...
            if FIR_med:
                x_connect, y_connect = self.median_sed(median_x5, median_y5, connect_point=True, Bin=True, bin_in=b5, lw=3)
                self.median_FIR_sed(wfir4, ffir4, connect=[x_connect, y_connect], upper=FIR_upper, Norm=False, Bin=True, bin_in=b5, lw=3)
            else:
